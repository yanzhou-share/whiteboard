...
fabric.Collection = {
  ...
  // backspace delete group selection except iamge element
  removeGroup: function() {
    var objects = this.getObjects(),
        index, obj;


    for (var i = 0, length = arguments[0].length; i < length; i++) {
      obj = arguments[0][i];
      //if(!!obj && typeof obj.optype != "undefined") obj = obj.toJSON();
      //!!obj && typeof obj.optype != "undefined" && delete obj.optype;
      //objects = this.toObjects(objects);
      ...
      // only call onObjectRemoved if an object was actually removed
      if (index !== -1 && obj.type != 'image') {//except image because group do not delete image
        objects.splice(index, 1);
        this._onObjectRemoved(arguments[0][i], 'eraser');
      }
    }

    this.renderOnAddRemove && this.renderAll("eraser");
    return this;
  },
...
(function(global) {
  ...
  fabric.util = {
     ...
     */
    rotatePoint: function(point, origin, radians,obj) {
        // if (!(typeof obj == 'undefined')) {
        //   origin.x = fabric.StaticCanvasWidth*origin.x/(obj.width/obj.RatioOffset[0]);
        //   origin.y = fabric.StaticCanvasHeight*origin.y/(obj.height/obj.RatioOffset[1]);
        //   point.x = fabric.StaticCanvasWidth*point.x/(obj.width/obj.RatioOffset[0]);
        //   point.y = fabric.StaticCanvasHeight*point.y/(obj.height/obj.RatioOffset[1]);
        // } 
        // else {  
        //   // origin.x = obj.left;
        //   // origin.y = obj.top;     
...
(function () {
  ...
  fabric.StaticCanvas = fabric.util.createClass(fabric.CommonMethods,/** @lends fabric.StaticCanvas.prototype */ {
    ...
    getBoundingRectLeftAndTop : function(){
      return getElementOffset(this.lowerCanvasEl);
    },
    ...
    setWidth: function (value, options) {
      fabric.StaticCanvasWidth = value;
    ...
    setHeight: function (value, options) {
      fabric.StaticCanvasHeight = value;
    ...
    renderAll: function (type) {
      ...
      if (this.contextTop  && (type == "eraser" || this.selection || !!this.getActiveObject()) && !this._groupSelector && !this.isDrawingMode) {
          this.clearContext(this.contextTop);
          this.clearContext(canvasToDrawOn);
      }

      if(type == "blur" || type == "refresh" || type == "modified" || type =="removed" || type == "zoom" || type == "drag" || type == "undo" || type == "redo"){
        this.clearContext(canvasToDrawOn);
      }
      ...
      if (type == "eraserPath" || type == "rotateImage" || type == "removeImage" ||typeof type == "undefined" ) {
        this.clearContext(this.contextTop);
        this.clearContext(canvasToDrawOn);
      }
  ...
  fabric.PencilBrush = fabric.util.createClass(fabric.BaseBrush, /** @lends fabric.PencilBrush.prototype */ {
    ...
    convertPointsToSVGPath: function(points) {
      ...
      var rx = this.canvas.width,
          ry = this.canvas.height;

      //path.push('M ', points[0].x, ' ', points[0].y, ' ', points[0].x/rx, ' ', points[0].y/ry);
      //for (var i = 1, len = points.length; i < len; i++) {
      //  var midPoint = p1.midPointFrom(p2);
      //  // p1 is our bezier control point
      //  // midpoint is our endpoint
      //  // start point is p(i-1) value.
      //  path.push('Q ', p1.x, ' ', p1.y, ' ', midPoint.x, ' ', midPoint.y, ' ',p1.x/rx, ' ', p1.y/ry, ' ', midPoint.x/rx, ' ', midPoint.y/ry);
      //  p1 = new fabric.Point(points[i].x, points[i].y);
      //  if ((i + 1) < points.length) {
      //    p2 = new fabric.Point(points[i + 1].x, points[i + 1].y);
      //  }
      //}
      //path.push('L ', p1.x, ' ', p1.y, ' ', p1.x/rx, ' ', p1.y/ry);
      //return path;
    ...
    _finalizeAndAddPath: function() {
      ...
      path.scale = this.canvas.width/this.canvas.height;
...
(function() {
  ...
  fabric.Canvas = fabric.util.createClass(fabric.StaticCanvas, /** @lends fabric.Canvas.prototype */ {
    ...
    setStyleCursor : function(cs){
      var cls = this.upperCanvasEl.className,
          clsArr = cls.split(' ');
      
      this.upperCanvasEl.setAttribute('class',clsArr[0] + ' ' + cs);
      //fabric.util.addClass(this.upperCanvasEl, cls);
    },
...
(function() {
  ...
  fabric.util.object.extend(fabric.Canvas.prototype, /** @lends fabric.Canvas.prototype */ {
    ...
    __onMouseDown: function (e) {
      ...
      var c_type = !target ? "blur" : "";
      // we must renderAll so that active image is placed on the top canvas
      shouldRender && this.renderAll(c_type);
...
fabric.util.object.extend(fabric.StaticCanvas.prototype, /** @lends fabric.StaticCanvas.prototype */ {
  ...
  loadFromJSON : function(json, callback, reviver){
      ...
      if (json.objects) {
          ...
          //     if(this._objects[j].id == json.objects[i]){
          //       objArray.push(this._objects[j]);
          //     }
          //   }
          // }
          this._objects.forEach(function(obj) {
            for (var i = 0; i < json.objects.length; i++) {
              if (obj.id == json.objects[i].id) {
                objArray.push(obj);
              }
            }
          });
      ...
      // if(typeof json.ptype != "undefined" && json.ptype == "removed"){
      //     this.clear();
      // }
      return this;
  },
  ...
  _enlivenObjects: function (objects, callback, reviver) {
    ...
    fabric.util.enlivenObjects(objects, function(enlivenedObjects) {
      enlivenedObjects.forEach(function(obj, index) {
        ...
        if (obj.type == 'image' && typeof fabric.otherCanvasHeight != 'undefined' && typeof fabric.otherCanvasWidth != 'undefined') {
          //解决图片进行拖动，缩放时的比例问题
          obj.left = fabric.StaticCanvasWidth*obj.left/fabric.otherCanvasWidth;
          obj.top = fabric.StaticCanvasHeight*obj.top/fabric.otherCanvasHeight;
        }else if (obj.type == 'image'){
          //解决刷新后，图片位置变化的Bug，因为left，top都是存的操作一方的，
          //所以，canvasWidth，和canvasHeight也应该是存的操作的一方的
          obj.left = fabric.StaticCanvasWidth*obj.left/obj.canvasWidth;
          obj.top = fabric.StaticCanvasHeight*obj.top/obj.canvasHeight;
        }
...
(function(global) {
  ...
  fabric.Object = fabric.util.createClass(/** @lends fabric.Object.prototype */ {
     ...
     * @default
     */
    cornerColor:              'rgba(102,153,255,0.9)',
    ...
    toObject: function(propertiesToInclude) {
      var NUM_FRACTION_DIGITS = fabric.Object.NUM_FRACTION_DIGITS,
          ...
          object = {
            ...
            canvasWidth:fabric.StaticCanvasWidth,
            canvasHeight:fabric.StaticCanvasHeight,
            pathData: fabric.pathDataInfo
          };
  ...
  fabric.util.object.extend(fabric.Object.prototype, /** @lends fabric.Object.prototype */ {
    ...
    translateToGivenOrigin: function(point, fromOriginX, fromOriginY, toOriginX, toOriginY) {
      ...
      if (offsetX || offsetY) {
        ...
        if(this.type == 'image'){
           // x = fabric.StaticCanvasWidth * x /(this.width/this.RatioOffset[0]) + offsetX * dim.x;
           // y = fabric.StaticCanvasHeight * y /(this.height/this.RatioOffset[1]) + offsetY * dim.y;
            x += offsetX * dim.x;
            y += offsetY * dim.y;
              // if (fabric.StaticCanvasWidth ===(this.width/this.RatioOffset[0]) ) {
              //     x = fabric.StaticCanvasWidth * x /(this.width/this.RatioOffset[0]) + offsetX * dim.x;
              //     y = fabric.StaticCanvasHeight * y /(this.height/this.RatioOffset[1]) + offsetY * dim.y;
              //  }else{
              ...
              //           break;
              //       case 1:
              //           x = fabric.StaticCanvasWidth * x /(this.width/this.RatioOffset[0]) - offsetX * dim.y;
              //           y = fabric.StaticCanvasHeight * y /(this.height/this.RatioOffset[1]) + offsetY * dim.x;
              //           break;
              //       case 2:
              //           x = fabric.StaticCanvasWidth * x /(this.width/this.RatioOffset[0]) - offsetX * dim.x;
              //           y = fabric.StaticCanvasHeight * y /(this.height/this.RatioOffset[1]) - offsetY * dim.y;
        ...
        } else if(!!this.RatioOffset){
          x = fabric.StaticCanvasWidth * x /this.RatioOffset[2] + offsetX * dim.x;
          y = fabric.StaticCanvasHeight * y /this.RatioOffset[3] + offsetY *dim.y;
        } else {
    ...
    translateToCenterPoint: function(point, originX, originY) {
      var p = this.translateToGivenOrigin(point, originX, originY, 'center', 'center');
        ...
        // if (typeof this.RatioOffset != 'undefined') {
        //   point.x = fabric.StaticCanvasWidth*point.x/(this.width/this.RatioOffset[0]);
        //   point.y = fabric.StaticCanvasHeight*point.y/(this.height/this.RatioOffset[1]);
        // }
      if (this.angle) {
        return fabric.util.rotatePoint(p, point, degreesToRadians(this.angle),this);
      }
...
fabric.util.object.extend(fabric.Object.prototype, /** @lends fabric.Object.prototype */ {
  ...
  saveState: function(options) {
    this.stateProperties.forEach(function(prop) {
        //当同步端选中图片时,该属性为undefined
        //所以 手动给他赋值
        if(typeof this.originalState == "undefined"){
            this.originalState = {};
        }
  ...
  fabric.util.object.extend(fabric.Object.prototype, /** @lends fabric.Object.prototype */ {
    ...
    _findTargetCorner: function(pointer) {
      ...
      for (var i in this.oCoords) {
        if(this.type=='i-text' && ['tl','tr','bl','br','mt','mr','mb','ml'].indexOf(i)!=-1){
          continue;
        }
    ...
    _getNonTransformedDimensions: function() {
      ...
      if(this.type == 'image'){
        w = fabric.StaticCanvasWidth * this.RatioOffset[0];
        h = fabric.StaticCanvasHeight * this.RatioOffset[1];
      }
      // else if (this.type == "i-text" && !!this.RatioOffset) {
      //   //为以后按照比例求文字的中心做基础
      //   w = w/this.RatioOffset[2]*fabric.StaticCanvasWidth;
      //   h = h/this.RatioOffset[3]*fabric.StaticCanvasHeight;
      // }
...
(function(global) {
  ...
  fabric.Path = fabric.util.createClass(fabric.Object, /** @lends fabric.Path.prototype */ {
     ...
     */
    initialize: function(path, options,strFromAddPath) {
      options = options || { };
      ...
      this.path = fromArray
        ? path
        // one of commands (m,M,l,L,q,Q,rc,C,etc.) followed by non-command characters (i.e. command values)
      ...
      if (!fromArray) {
        ...
        /*
        解决撤销线条操作时,pathRatio不正确问题
         */
        if (typeof strFromAddPath == "undefined") {
          this.path = this._parsePath();
          this.pathRatio = this._parsePathRatio();
        }else if (strFromAddPath == "asy") {
          this.path = this.pathRatio;
        }
    ...
    _setPositionDimensions: function(options) {
      ...
      this.pathOffsetRadio = {
        x: !!fabric.StaticCanvasWidth ? this.pathOffset.x/fabric.StaticCanvasWidth : 1,
        y: !!fabric.StaticCanvasHeight ? this.pathOffset.y/fabric.StaticCanvasHeight : 1
      };
    },
    ...
    _render: function(ctx) {
      var current, // current instruction
          ...
          l = -this.pathOffset.x,
          t = -this.pathOffset.y,
          R_width = this.canvas.width,
          R_height = this.canvas.height;
          // l = -this.pathOffsetRadio.x*R_width,
          // t = -this.pathOffsetRadio.y*R_height;
      ...
      if(typeof this.pathRatio != "undefined"){
        for (var i = 0, len = this.pathRatio.length; i < len; ++i) {

          current = this.pathRatio[i];

          switch (current[0]) { // first letter

            case 'l': // lineto, relative
              x += current[1]*R_width;
              y += current[2]*R_height;
              ctx.lineTo(x + l, y + t);
              break;
            ...
            case 'c': // bezierCurveTo, relative
              tempX = x + current[5]*R_width;
              tempY = y + current[6]*R_height;
              controlX = x + current[3]*R_width;
              controlY = y + current[4]*R_height;
              ctx.bezierCurveTo(
                  x + current[1]*R_width + l, // x1
                  y + current[2]*R_height + t, // y1
                  controlX + l, // x2
                  controlY + t, // y2
                  tempX + l,
                  tempY + t
            ...
            case 'C': // bezierCurveTo, absolute
              x = current[5]*R_width;
              y = current[6]*R_height;
              controlX = current[3]*R_width;
              controlY = current[4]*R_height;
              ctx.bezierCurveTo(
                  current[1]*R_width + l,
                  current[2]*R_height + t,
                  controlX + l,
                  controlY + t,
                  x + l,
                  y + t
            ...
            case 's': // shorthand cubic bezierCurveTo, relative
              ...
              tempX = x + current[3]*R_width;
              tempY = y + current[4]*R_height;
              ...
              ctx.bezierCurveTo(
                  controlX + l,
                  controlY + t,
                  x + current[1]*R_width + l,
                  y + current[2]*R_height + t,
                  tempX + l,
                  tempY + t
              );
              ...
              controlX = x + current[1]*R_width;
              controlY = y + current[2]*R_height;
            ...
            case 'S': // shorthand cubic bezierCurveTo, absolute
              tempX = current[3]*R_width;
              tempY = current[4]*R_height;
              if (previous[0].match(/[CcSs]/) === null) {
                // If there is no previous command or if the previous command was not a C, c, S, or s,
                // the control point is coincident with the current point
                controlX = x;
              ...
              // the reflection of the second control point on
              // the previous command relative to the current point."
              controlX = current[1]*R_width;
              controlY = current[2]*R_height;
            ...
            case 't': // shorthand quadraticCurveTo, relative
              ...
              // transform to absolute x,y
              tempX = x + current[1]*R_width;
              tempY = y + current[2]*R_height;
            ...
            case 'T':
              tempX = current[1]*R_width;
              tempY = current[2]*R_height;
            ...
            case 'a':
              ...
              drawArc(ctx, x + l, y + t, [
                ...
                current[4]*R_height,
                current[5]*R_width,
                current[6]*R_height + x + l,
                current[7]*R_width + y + t
              ]);
              x += current[6]*R_height;
              y += current[7]*R_width;
              break;
    ...
    toObject: function(propertiesToInclude) {
      var o = extend(this.callSuper('toObject', propertiesToInclude), {
        ...
        pathOffsetRadio : this.pathOffsetRadio,
        leftRadio: this.leftRadio,
        topRadio: this.topRadio,
    ...
    _parsePathRatio: function() {
      ...
      for (var i = 0, coordsParsed, len = this.path2.length; i < len; i++) {
        currentPath = this.path2[i];
        ...
        for (var j = 0, jlen = coords.length; j < jlen; j++) {
          ...
          if(j%2 == 0){
            parsed = parsed/this.canvasWidth;
          }else{
            parsed = parsed/this.canvasHeight;
          }
          if (!isNaN(parsed)) {
...
(function(global) {
  ...
  fabric.Image = fabric.util.createClass(fabric.Object, /** @lends fabric.Image.prototype */ {
    ...
    initialize: function(element, options) {
      ...
      if(typeof options.RatioOffset == 'undefined'){
        this._parseImageRatio(options);
      } else {
        this.RatioOffset = options.RatioOffset;
      }
    },

    _parseImageRatio : function(options){
      var Ratioleft = options.left / fabric.StaticCanvasWidth,
          Ratiotop = options.top / fabric.StaticCanvasHeight,
          Ratiowidth = options._width / fabric.StaticCanvasWidth,
          Ratioheight = options._height / fabric.StaticCanvasHeight;
          

      this.RatioOffset = [Ratiowidth, Ratioheight, Ratioleft, Ratiotop];
    },
    ...
    _render: function(ctx, noTransform) {
      ...
      x = (noTransform ? this.left : -fabric.StaticCanvasWidth * this.RatioOffset[0] / 2);
      y = (noTransform ? this.top : -fabric.StaticCanvasHeight * this.RatioOffset[1] / 2);
      // x = (noTransform ? this.left : -this.width / 2);
      // y = (noTransform ? this.top : -this.height / 2);
      ...
      elementToDraw && ctx.drawImage(elementToDraw,
                                    -fabric.StaticCanvasWidth * this.RatioOffset[0]/2,
                                    -fabric.StaticCanvasHeight * this.RatioOffset[1]/2,
                                    // fabric.StaticCanvasWidth * this.RatioOffset[2],
                                    // fabric.StaticCanvasHeight * this.RatioOffset[3],
                                     // x + imageMargins.marginX,
                                     // y + imageMargins.marginY,
                                     // imageMargins.width,
                                     // imageMargins.height
                                     fabric.StaticCanvasWidth * this.RatioOffset[0],
                                     fabric.StaticCanvasHeight * this.RatioOffset[1]
                                    );
...
(function(global) {
  ...
  fabric.Text = fabric.util.createClass(fabric.Object, /** @lends fabric.Text.prototype */ {
    ...
    _initDimensions: function(ctx) {
      ...
      //通过比例计算框的width和height
      this.width = this.RatioOffset? this._getTextWidth(ctx)/this.RatioOffset[2]*fabric.StaticCanvasWidth:this._getTextWidth(ctx);
      this.height = this.RatioOffset? this._getTextHeight(ctx)/this.RatioOffset[3]*fabric.StaticCanvasHeight:this._getTextHeight(ctx);
    },
    ...
    _getLineWidth: function(ctx, lineIndex) {
      if (this.__lineWidths[lineIndex]) {
        ...
        return this.__lineWidths[lineIndex] === -1 ? this.width : this.__lineWidths[lineIndex]/this.RatioOffset[2]*fabric.StaticCanvasWidth;
      }
    ...
    _getFontDeclaration: function() {
      return [
       ...
       // this.fontSize + 'px',
       // 通过比例计算出相应的字号
       (this.RatioOffset ?this.fontSize/this.RatioOffset[2]*fabric.StaticCanvasWidth + 'px' :this.fontSize + 'px'),
...
(function() {
  ...
  fabric.IText = fabric.util.createClass(fabric.Text, fabric.Observable, /** @lends fabric.IText.prototype */ {
    ...
    initialize: function(text, options) {
      ...
      this.initBehavior();
      if (typeof options.scaleNum == 'undefined') {
       // this.RatioOffset = [options.left/fabric.StaticCanvasWidth,options.top/fabric.StaticCanvasHeight,fabric.StaticCanvasWidth,fabric.StaticCanvasHeight];
        this.RatioOffset = [options.left/fabric.StaticCanvasWidth,options.top/fabric.StaticCanvasHeight,fabric.StaticCanvasWidth,fabric.StaticCanvasHeight];
      } else {
        this.RatioOffset = options.scaleNum;
      }
    ...
    _getHeightOfLine: function(ctx, lineIndex) {
      if (this.__lineHeights[lineIndex]) {
        ...
        //按照比例等比的计算文字在框内的位置(变态的5)
        return (this.RatioOffset ? this.__lineHeights[lineIndex]/this.RatioOffset[3]*fabric.StaticCanvasHeight:this.__lineHeights[lineIndex]) + 5;
      }
...
(function() {
  ...
  fabric.util.object.extend(fabric.IText.prototype, /** @lends fabric.IText.prototype */ {
    ...
    insertChar: function(_char, skipUpdate, styleObject) {
      ...
      // console.log('this.text:'+this.text+',char:'+_char);
      // console.log('this.selectionStart:'+this.selectionStart+',this.selectionEnd:'+this.selectionEnd);
...
fabric.util.object.extend(fabric.IText.prototype, /** @lends fabric.IText.prototype */ {
  ...
  onKeyUp: function(e) {
    ...
    console.log('onKeyUp ' + this.hiddenTextarea.value);
  ...
  onInput: function(e) {
    ...
    //如果是高版本浏览器，则执行原来的操作
    if(sys.chrome >= '57' || sys.opera >= "34" || sys.jisu >= "8.5"){
        // console.log('this.text:'+this.text+',newText:'+this.hiddenTextarea.value);
        if (newTextLength > textLength) {
          //we added some character
          start = this._selectionDirection === 'left' ? offsetEnd : offset;
          diff = newTextLength - textLength;
          charsToInsert = this.hiddenTextarea.value.slice(start, start + diff);
          console.log('charsToInsert:'+charsToInsert);
        }
        else {
        ...
        // this.canvas.getActiveObject().setText(this.hiddenTextarea.value);
        // this.canvas.renderAll();
  ...
  */
  ...
  onCompositionUpdate: function(e) {
    var data = e.data;
    // console.log('onCompositionUpdate:' + data);
...
fabric.Object.prototype.url = "";
fabric.Object.prototype.ppts = [];
